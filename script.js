...
informations.forEach((information) => information.count = 0);
...
const confirmedPart = document.querySelector(".confirmed-part");
...
let added = false;
let totalPrice = 0;
let countingCartNumber = 0;
// #########################################################
// All Functions


const deleteAPurchase = function(e){
  if(e.target.classList.contains("close-btn")) {
    const orderName = e.target.previousElementSibling.firstElementChild.textContent;
    informations.forEach((item) => {
      if(item.category === orderName) item.count = 0;
    })
    cart = cart.filter((item) => item.category !== orderName);
    console.log(cart);
    // e.target.parentElement.style.display = "none";
    decrease(informations);
    productBtn.forEach((btn) => {
      if(btn.parentElement.children[3].children[0].textContent === orderName)
        btn.parentElement.children[2].children[1].textContent = 0;
    })
  }
}
...
const increase = function(item){
    ...
    cart = new Array();
    cart = informations.filter((information) => information.count > 0);
    addToCart();
    console.log(cart);
}
...
const decrease = function(item){
  ...
  if(cart.length === 0) {
    ...
    `
    toAdd.insertAdjacentHTML("afterbegin",html1);
    return;
  }
  addToCart();
  console.log(cart);
...
const addToCart = function(){
    ...
    countingCartNumber = 0;
    totalPrice = 0;
    cart.forEach((item) => {
        const html1 = `
      <div class="cart-purchase">
        <div class="all-details">
          <div class="secondary-title">${item.category}</div>
          <div class="prices">
            <span class="number-of-purchases">${item.count}x</span>
            <span class="individual-purchases"
              ><span class="at">@</span>${item.price}</span
            >
            <span class="total-purchases">$${item.price * item.count}</span>
          </div>
        </div>
        <svg
          class="close-btn btn"
        ...
        toAdd.insertAdjacentHTML("beforeEnd",html1);
        totalPrice += (item.price * item.count);
        countingCartNumber += item.count;
    });
    totalPriceContainer.textContent = '';
        const html2 = `
            <div class="total-price-container">
        <div class="price-title">Order Total:</div>
      ...
      <button class="confirm-btn btn">Confirm Order</button>
        ...
        toAdd.insertAdjacentHTML("beforeend",html2);
        cartNumber.textContent = countingCartNumber;
    }

    const confirmTheOrder = function(e){
        if(e.target.classList.contains("confirm-btn")) {
          confirmedPart.classList.remove("hidden");
          overlay.classList.remove("hidden");
          cart.forEach((item) => {
            const html1 = `
            <div class="confirmed-product">
              <div class="confirmed-flex">
                <img src="${item.image.thumbnail}" class="confirmed-image" alt="${item.category}">
                   <div class="confirmed-details">
                     <div class="confirmed-secondary-title">${item.category}</div>
                      <div class="prices">
                      <span class="number-of-purchases">${item.count}x</span>
                       <span class="individual-purchases"><span class="at">@</span>$${item.price}</span>
                   </div>
                </div>
               </div>
                <span class="total-price-confirmed">$${item.price * item.count}</span>
            </div>
          `
          confirmedProducts.insertAdjacentHTML("beforeend",html1);
          });
          ...
          const html2 = `
            ...
            <div class="total-price">$${totalPrice}</div>
          `
          confirmedTotal.insertAdjacentHTML("beforeEnd",html2);
        }
    }

    const newOrder = function(e){
      if(e.target.classList.contains("new-order-btn") || e.target.classList.contains("overlay")){
        productBtn.forEach((btn) => {
          btn.nextElementSibling.classList.add("opacity");
          btn.classList.remove("hidden");
          selectText.forEach((select) => select.textContent = '0');
        })
        overlay.classList.add("hidden");
        confirmedPart.classList.add("hidden");
        informations.forEach((item) => item.count = 0);
        cart = new Array();
        decrease(informations);
        product.forEach((border) => border.firstElementChild.style.border = "none");
      }
    }
//##########################################################
...
products.addEventListener("click",function(e){
    ...
    e.target.previousElementSibling.style.border = "2px solid var(--color-red)";
})
...
products.addEventListener("click",function(e){
    if(!e.target.classList.contains("increase-btn") && !e.target.classList.contains("decrease-btn")) return;

    if(e.target.classList.contains("increase-btn")){
        const dataName = e.target.closest(".clicked-btn").nextElementSibling.firstElementChild.textContent;
        e.target.previousElementSibling.textContent++;
        informations.forEach((information) =>{
          if(information.category === dataName) increase(information);
        })
    ...
    if(e.target.classList.contains("decrease-btn")){
      ...
      informations.forEach((information) => {
        if(information.category === dataName) decrease(information);
      })

    }
});

theCart.addEventListener("click",deleteAPurchase);
theCart.addEventListener("click",confirmTheOrder);
confirmedPart.addEventListener("click",newOrder);
overlay.addEventListener("click",newOrder);
